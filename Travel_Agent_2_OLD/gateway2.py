...
class GatewayService2:
    ...
    travel_agent_rpc2 = RpcProxy('travel_agent_2')

    @http('GET', '/travel-agent-2')
    def get_travel_agent(self, request):
        travel_agent = self.travel_agent_rpc2.get_travel_agent()
        return (200, self.header, json.dumps(travel_agent))

    @http('GET', '/travel-agent-2/packages')
    def get_packages(self, request):
        packages = self.travel_agent_rpc2.get_packages()
        return (200, self.header, json.dumps(packages))

    @http('GET', '/travel-agent-2/package_details')
    def get_all_package_details(self, request):
        package_details = self.travel_agent_rpc2.get_all_package_details()
        return (200, self.header, json.dumps(package_details))
    
    @http('GET', '/travel-agent-2/package/<int:id>')
    def get_package_by_id(self, request, id):
        package = self.travel_agent_rpc2.get_package_by_id(id)
        return (200, self.header, json.dumps(package))
    
    @http('GET', '/travel-agent-2/package_details/<int:package_id>')
    def get_package_details(self, request, package_id):
        package_details = self.travel_agent_rpc2.get_package_details(package_id)
        return (200, self.header, json.dumps(package_details))
    
    @http('GET', '/travel-agent-2/package/<int:package_id>/day/<int:day>')
    ...
    @http('GET', '/travel-agent-2/bookings')
    ...
    @http('POST', '/travel-agent-2/book')
    def add_booking(self, request):
        data = json.loads(request.get_data(as_text=True))
        user_id = data['user_id']
        package_id = data['package_id']
        booking = self.travel_agent_rpc2.add_booking(user_id, package_id)
        return (200, self.header, json.dumps(booking))
    
    @http('GET', '/travel-agent-2/filter')
    def filter_packages(self, request):
        ...
        filtered_packages = self.travel_agent_rpc2.filter(origin_city, destination_city, number_of_people, departure_date, return_date)
