document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchBookingsData() {
        const response = await fetch(`http://3.210.234.45:8003/travel-agent/bookings/${userID}`, {
            method: "GET"
        });
    ...
    async function fetchPackagesData() {
        try {
            ...
            if (originCity == null || destinationCity == null || minimimNumberOfPerson == null || departureDate == null || returnDate == null) {
                response = await fetch('http://3.210.234.45:8003/travel-agent/packages', {
                    method: "GET"
                });
            } else {
                response = await fetch(`http://3.210.234.45:8003/travel-agent/filter?origin_city=${originCity}&destination_city=${destinationCity}&number_of_people=${minimimNumberOfPerson}&departure_date=${departureDate}&return_date=${returnDate}`, {
                    method: "GET",
                });
            }
    ...
    fetchPackagesData().then(function () {
        ...
        detailBtn.forEach(element => {
            element.addEventListener("click", async function () {
                ...
                const response = await fetch(`http://3.210.234.45:8003/travel-agent/package_details/${packageId}`, {
                    method: "GET"
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
