document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchBookingsData() {
        const response = await fetch('http://localhost:8000/travel-agent-3/bookings', {
            method: "GET"
        });
    ...
    async function fetchPackagesData() {
        try {
            ...
            if (originCity == null || destinationCity == null || minimimNumberOfPerson == null || departureDate == null || returnDate == null) {
                response = await fetch('http://localhost:8000/travel-agent-3/packages', {
                    method: "GET"
                });
            } else {
                response = await fetch(`http://localhost:8000/travel-agent-3/filter?origin_city=${originCity}&destination_city=${destinationCity}&number_of_people=${minimimNumberOfPerson}&departure_date=${departureDate}&return_date=${returnDate}`, {
                    method: "GET",
                });
            }
    ...
    fetchPackagesData().then(function () {
            ...
            button.addEventListener("click", function () {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const response = await fetch(`http://localhost:8000/travel-agent-3/book`, {
                            method: "POST",
                            body: JSON.stringify({
                                user_id: "1",
        ...
        detailBtn.forEach(element => {
            element.addEventListener("click", async function () {
                ...
                const response = await fetch(`http://localhost:8000/travel-agent-3/package_details/${packageId}`, {
                    method: "GET"
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
